...
(function (global) {
  ...
  function createPlanMindMap(deps) {
    ...
    function applyMindMapServerPayload(res) {
    ...
    async function loadTaskMindMapData() {
      try {
        ...
        return applyMindMapServerPayload(res);
      } catch (e) {
    ...
    async function applyPrefetchedRow(row) {
      if (mindMapLoaded) return mindMapStore;
      try {
        if (!row) return applyMindMapServerPayload({ mapData: null, updatedAt: null });
        const mapData = buildMindMapStoreFromApiRow(row);
        return applyMindMapServerPayload({ mapData, updatedAt: row.updated_at || null });
      } catch (e) {
        console.warn('[mindmap] 预取数据应用失败', e.message || e);
        return loadTaskMindMapData();
      }
    }
